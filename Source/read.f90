...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            MASS_FRACTION_COND_0,MASS_FRACTION_0,ODE_REL_ERROR,ODE_ABS_ERROR,&
...
SPEC_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   ELSEIF (SM%H_F > -1.E21_EB) THEN
      ...
      IF(N .EQ. 1)WRITE(LU_ERR,*)"INSIDE PROC_SMIX - SM%H_F=",SM%H_F
      SM%H_F = SM%H_F / SM%MW * 1000._EB
      IF(N .EQ. 1)WRITE(LU_ERR,*)"INSIDE PROC_SMIX2 - SM%H_F=",SM%H_F
...
CHARACTER(FORMULA_LENGTH) :: FORMULA
CHARACTER(255) :: EQUATION,TEMP_EQUATION='', REACTYPE
...
NAMELIST /REAC/ A,AIT_EXCLUSION_ZONE,AIT_EXCLUSION_ZONE_CTRL_ID,AIT_EXCLUSION_ZONE_DEVC_ID,AUTO_IGNITION_TEMPERATURE,&
                ...
                SPEC_ID_N_S,SPEC_ID_NU,THIRD_BODY,THIRD_EFF_ID,THIRD_EFF, REACTYPE, A_LOW_PR, E_LOW_PR,N_T_LOW_PR, &
                A_TROE, T1_TROE, T2_TROE, T3_TROE
...
WRITE(LU_ERR,*)"N_TRACKED_SPECIES,N_REACTIONS=",N_TRACKED_SPECIES,N_REACTIONS
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   ! Handle reactions with same species on both the side, such as CH3O + H = CH2OH + H
   ! Make these reactions as THIRD_BODY reactions
   DO NS=1,RN%N_SMIX
      IF (TRIM(RN%SPEC_ID_NU_READ(NS))=='null') CYCLE
      DO NS2=1,N_TRACKED_SPECIES
         IF (TRIM(RN%SPEC_ID_NU_READ(NS))==TRIM(SPECIES_MIXTURE(NS2)%ID)) THEN
            ...
            IF (RN%N_THIRD>0) THEN
              DO NS3=1,RN%N_THIRD
                IF (RN%THIRD_EFF_ID_READ(NS3)==TRIM(RN%SPEC_ID_NU_READ(NS))) THEN
                   NAME_FOUND = .TRUE.
                ENDIF   
              ENDDO
            ENDIF
            IF (RN%NU(NS2) .EQ. 0._EB .AND. .NOT. NAME_FOUND ) THEN
              RN%N_THIRD = RN%N_THIRD +1
              IF (.NOT. ALLOCATED(RN%THIRD_EFF_ID_READ)) THEN
                 ALLOCATE(RN%THIRD_EFF_ID_READ( RN%N_THIRD)) ! Assuming only one species for the time being.
                 ALLOCATE(RN%THIRD_EFF_READ(RN%N_THIRD))
              ENDIF   
              RN%THIRD_EFF_ID_READ(RN%N_THIRD) = TRIM(RN%SPEC_ID_NU_READ(NS))
              RN%THIRD_EFF_READ(RN%N_THIRD) = 1.0_EB
              RN%REACTYPE='THREE-BODY-ARRHENIUS'
              !WRITE(LU_ERR,*)"RN%ID, RN%THIRD_EFF_ID_READ, RN%THIRD_EFF_READ=",RN%ID, RN%THIRD_EFF_ID_READ, RN%THIRD_EFF_READ
            ENDIF
         ENDIF
      ENDDO   
   ENDDO
   ...
   IF (RN%N_THIRD>0) THEN
      ...
      RN%THIRD_EFF = 1._EB
      IF (RN%N_THIRD .EQ. 1) RN%THIRD_EFF = 0._EB ! Single gas species
      DO NS=1,RN%N_THIRD
         NAME_FOUND = .FALSE.
