...
REAL(EB):: MASS_FRACTION(MAX_SPECIES),VOLUME_FRACTION(MAX_SPECIES),MIN_DIAMETER,MAX_DIAMETER,&
           POLYNOMIAL_TEMP(1:4),POLYNOMIAL_COEFF(9,3)
...
NAMELIST /SPEC/ AEROSOL,BACKGROUND,BETA_LIQUID,C,CONDUCTIVITY,CONDUCTIVITY_LIQUID,CONDUCTIVITY_SOLID,COPY_LUMPED, &
                ...
                MELTING_TEMPERATURE,MIN_DIAMETER,MW,N,N_BINS,O,POLYNOMIAL,POLYNOMIAL_TEMP,POLYNOMIAL_COEFF,PR_GAS,PRIMITIVE,&
                RADCAL_ID,RAMP_CP,RAMP_CP_L,RAMP_D,RAMP_G_F,RAMP_K,RAMP_MU,REFERENCE_ENTHALPY,REFERENCE_TEMPERATURE,SIGMALJ,&
                SPEC_ID,SPECIFIC_HEAT,SPECIFIC_HEAT_LIQUID,THERMOPHORETIC_DIAMETER,VAPORIZATION_TEMPERATURE,VISCOSITY,&
                VISCOSITY_LIQUID,VOLUME_FRACTION
...
PRIMITIVE_LOOP: DO N1=1,N_SPECIES - N_COPY_PRIMITIVE
   ...
   SS%POLY_ID=POLYNOMIAL
   IF (SS%POLY_ID/='null') THEN
      IF (POLYNOMIAL_TEMP(1) < -1.E29 .AND. POLYNOMIAL_TEMP(2) < -1.E29) THEN
         WRITE(MESSAGE,'(A,A,A)') 'ERROR: SPEC ',TRIM(ID),' POLYNOMIAL_TEMP must have at least two values'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      SS%BANDS=1
      IF (POLYNOMIAL_TEMP(3) > -1.E29) SS%BANDS=2
      IF (POLYNOMIAL_TEMP(4) > -1.E29) SS%BANDS=3
      ALLOCATE(SS%POLY_T(SS%BANDS+1))
      SS%POLY_T(1:SS%BANDS+1) = POLYNOMIAL_TEMP(1:SS%BANDS+1)
      IF (POLYNOMIAL=='NASA7') THEN
         ALLOCATE(SS%POLY_A(7,SS%BANDS))
         DO NN=1,SS%BANDS
            DO NNN=1,7
               IF (POLYNOMIAL_COEFF(NNN,NN) <-1.E29_EB) THEN
                  WRITE(MESSAGE,'(A,A,A)') 'ERROR: SPEC ',TRIM(ID),' In sufficient values for POLYNOMIAL_COEFF.'
                  CALL SHUTDOWN(MESSAGE) ; RETURN
               ENDIF
               SS%POLY_A(NNN,NN) = POLYNOMIAL_COEFF(NNN,NN)
            ENDDO
         ENDDO
      ELSEIF (POLYNOMIAL=='NASA9') THEN
         ALLOCATE(SS%POLY_A(9,SS%BANDS))
         DO NN=1,SS%BANDS
            DO NNN=1,9
               IF (POLYNOMIAL_COEFF(NNN,NN) <-1.E29) THEN
                  WRITE(MESSAGE,'(A,A,A)') 'ERROR: SPEC ',TRIM(ID),' In sufficient values for POLYNOMIAL_COEFF.'
                  CALL SHUTDOWN(MESSAGE) ; RETURN
               ENDIF
               SS%POLY_A(NNN,NN) = POLYNOMIAL_COEFF(NNN,NN)
         ...
         ENDDO
      ELSE
         WRITE(MESSAGE,'(A,A,A)') 'ERROR: SPEC ',TRIM(ID),' POLYNOMIAL must be NASA7 or NASA9'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
   ...
   IF (SS%REFERENCE_ENTHALPY > -1.E21_EB .OR. SS%H_F > -1.E21_EB .OR. SS%BANDS>0) SS%EXPLICIT_H_F = .TRUE.
...
USE DASSL_CONSTANTS, ONLY : RN_AE, RN_ZZ
...
IF (TRIM(ODE_SOLVER)/='null') THEN
   SELECT CASE (TRIM(ODE_SOLVER))
      ...
      CASE ('DASSL') 
         IF (.NOT. ALL(SPECIES_MIXTURE%SINGLE_SPEC_INDEX>0)) THEN
            WRITE(MESSAGE,'(A)') 'ERROR: DASSL Solver require no lumped species.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         COMBUSTION_ODE_SOLVER = DASSL_SOLVER
         ALLOCATE(RN_AE(N_REACTIONS))
         ALLOCATE(RN_ZZ(1:N_TRACKED_SPECIES))
      CASE DEFAULT
         WRITE(MESSAGE,'(A)') 'ERROR: Problem with COMB. Name of ODE_SOLVER is not recognized.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
   END SELECT
ELSE
