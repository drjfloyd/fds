...
REACTION_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (COMBUSTION_ODE_SOLVER==DVODES_SOLVER) THEN
      WRITE(LU_OUTPUT,'(/6X,A,I3)')  'Relative error tolerance:  ', RICHARDSON_ERROR_TOLERANCE
      WRITE(LU_OUTPUT,'(/6X,A,I3)')  'Min. aboslute error tolerance:  ', DVODE_MIN_ATOL
   ENDIF
   IF (COMBUSTION_ODE_SOLVER==RK2_RICHARDSON) THEN
      WRITE(LU_OUTPUT,'(/6X,A,I3)')  'Richardson error tolerance:  ', RICHARDSON_ERROR_TOLERANCE
   ENDIF
