...
IF (N_FIXED_CHEMISTRY_SUBSTEPS>0 .AND. .NOT. COMBUSTION_ODE_SOLVER==DVODES_SOLVER) THEN
   WRITE(LU_OUTPUT,'(/6X,A,I3)')  'Number of Fixed Substeps:  ', N_FIXED_CHEMISTRY_SUBSTEPS
ENDIF
IF (COMBUSTION_ODE_SOLVER==DVODES_SOLVER) THEN
   WRITE(LU_OUTPUT,'(/6X,A,ES13.6)')  'Relative error tolerance:  ', RICHARDSON_ERROR_TOLERANCE
   WRITE(LU_OUTPUT,'(/6X,A,ES13.6)')  'Min. aboslute error tolerance:  ', DVODE_MIN_ATOL
ENDIF
IF (COMBUSTION_ODE_SOLVER==RK2_RICHARDSON) THEN
   WRITE(LU_OUTPUT,'(/6X,A,ES13.6)')  'Richardson error tolerance:  ', RICHARDSON_ERROR_TOLERANCE
ENDIF
