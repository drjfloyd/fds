...
INTEGER:: JACDIM, JACCOL = -2, MAXSTEPS=2000
...
!
!     IDID = -1, THE CODE HAS TAKEN ABOUT MAXSTEPS STEPS. IF YOU WANT TO CONTINUE, SET INFO(1) = 1 AND
!       CALL THE CODE AGAIN. AN ADDITIONAL MAXSTEPS STEPS WILL BE ALLOWED.
!
...
!
!     CHECK FOR TOO MANY STEPS
IF ((IWORK(LNST)-IWORK(LNSTL))<MAXSTEPS)GO TO 510
IDID=-1
GO TO 527
!
!     UPDATE WT
...
SELECT CASE (ITEMP)
   CASE(1)
      ...
      MESG = '***ERROR, AT CURRENT T (=R1) MAXSTEPS STEPS TAKEN ON THIS CALL BEFORE REACHING TOUT'
...
!
IF (IOPT < 1 .OR. IOPT > 2 .OR. N <= 0 .OR. TOL < ZERO .OR. LWA < (3*N**2 +13*N)/2) THEN
   ...
   IF (GNORM /= ZERO) THEN
      ...
      IF (SGNORM < DELTA) THEN
         ...
         TEMP = TEMP - (DELTA/QNORM)*(SGNORM/DELTA)**2 + &
                SQRT((TEMP-(DELTA/QNORM))**2 + (ONE-(DELTA/QNORM)**2)*(ONE-(SGNORM/DELTA)**2))
...
JDO: DO J = 1, MINMN
   ...
   IF (AJNORM /= ZERO) THEN
      ...
      IF (N >= JP1) THEN
         ...
         END DO
         IF (.NOT.PIVOT .OR. SIGMA(K) /= ZERO) THEN
            TEMP = A(J,K)/SIGMA(K)
...
FACTOR <= ZERO .OR. LDFJAC < N .OR. LR < (N*(N + 1))/2)  THEN
   CALL SNSQ_ERR
   RETURN
ENDIF
