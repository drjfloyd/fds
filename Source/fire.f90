...
! Set DVODES options
IF (COMBUSTION_ODE_SOLVER == DVODES_SOLVER .AND. .NOT. DVODE_INIT) THEN
   DVODE_INIT = .TRUE.
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (DVODES_SOLVER)
         ...
         DO NS =1,N_TRACKED_SPECIES
            ATOL(NS) = MAX(0.00000000000001D0,DBLE(RICHARDSON_ERROR_TOLERANCE*ZZ_MIXED(NS)))
         ENDDO
...
REAL(EB) :: DZ_F,DZZ_TMP(N_TRACKED_SPECIES),ZZ_TMP(N_TRACKED_SPECIES),X_Y(N_SPECIES),X_Y_SUM,T1
...
REACTION_LOOP: DO I=1,N_REACTIONS
   ...
   DZZ_TMP = 0._EB
   DO NS=1,RN%N_SMIX_FR
      DZZ_TMP(RN%NU_INDEX(NS)) = DZZ_TMP(RN%NU_INDEX(NS)) + RN%NU_MW_O_MW_F_FR(NS)*DZ_F
      DO NS2 = 1, RN%N_SPEC
         DZZ(RN%NU_INDEX(NS),YP2ZZ(RN%N_S_INDEX(NS2))) = DZZ(RN%NU_INDEX(NS),YP2ZZ(RN%N_S_INDEX(NS2))) + &
            DBLE(RN%N_S(NS2) * DZZ_TMP(RN%NU_INDEX(NS)) / ZZ_TMP(YP2ZZ(RN%N_S_INDEX(NS2))))
      ENDDO
   ENDDO
ENDDO REACTION_LOOP
