...
PUBLIC COMBUSTION,COMBUSTION_LOAD_BALANCED,COMBUSTION_BC,CONDENSATION_EVAPORATION,GET_FLAME_TEMPERATURE
   ...
   IF (CC_IBM) THEN
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         DO JCC=1,CUT_CELL(ICC)%NCELL
            ...
            CUT_CELL(ICC)%CHEM_ACTIVE(JCC) = .FALSE.
         ENDDO
      ENDDO
   ENDIF

   ! Set chemicaly active flag false for all the cells.
   CHEM_ACTIVE = .FALSE.
ENDDO
...
T_USED(10)=T_USED(10)+CURRENT_TIME()-TNOW-T_CHEM_ODE !subract off CHEM time so FIRE time doesn't double count
T_USED(16)=T_USED(16)+T_CHEM_ODE
...
USE CHEMCONS, ONLY: DO_CHEM_LOAD_BALANCE, LOAD_BALANCE_ALGO

INTEGER :: NM,I,J,K,CHEM_SUBIT_TMP,CHEM_SUBIT_TMP_OUT,ICC,JCC,INDX,NRECEIVE_CELLS
INTEGER :: NCHEM_ACTIVE_CELLS, NCHEM_ACTIVE_CELLS_GLOBAL, IERR
REAL(EB), INTENT(IN) :: T,DT
...
REAL(EB) :: MIX_TIME_OUT, Q_OUT, CHI_R_OUT, MYTEMP, MYRHO, MYMU, DELTA, VOL
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            CALL CHECK_CHEMICALLY_ACTIVE_STATE(ZZ_GET, TMP(I,J,K), I, J, K, IS_CHEM_ACTIVE)
   ...
   IF (CC_IBM) THEN
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO JCC=1,CC%NCELL
            ...
            PRES = PBAR(K,PRESSURE_ZONE(I,J,K)) + RHO(I,J,K)*(H(I,J,K)-KRES(I,J,K))
            CALL CHECK_CHEMICALLY_ACTIVE_STATE(ZZ_GET, CUT_CELL(ICC)%TMP(JCC), I, J, K, IS_CHEM_ACTIVE)
            IF (STOP_STATUS/=NO_STOP) RETURN
            IF (.NOT. IS_CHEM_ACTIVE) CYCLE
...
!------
 ...
 DO_CHEM_LOAD_BALANCE_IF : IF(N_MPI_PROCESSES > 1 .AND. DO_CHEM_LOAD_BALANCE) THEN
   IF (.NOT. ALLOCATED(NCHEM_ACTIVE_CELLS_RANK_WISE)) ALLOCATE(NCHEM_ACTIVE_CELLS_RANK_WISE(N_MPI_PROCESSES))
   NCHEM_ACTIVE_CELLS_RANK_WISE = 0
   CALL MPI_ALLGATHER(NCHEM_ACTIVE_CELLS, 1, MPI_INTEGER, NCHEM_ACTIVE_CELLS_RANK_WISE, 1, MPI_INTEGER, MPI_COMM_WORLD, IERR)
   NCHEM_ACTIVE_CELLS_GLOBAL = SUM(NCHEM_ACTIVE_CELLS_RANK_WISE(1:N_MPI_PROCESSES))

   IF (NCHEM_ACTIVE_CELLS_GLOBAL > 0) THEN
      IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) ALLOCATE(DZ_F0(N_REACTIONS))

      ! Distribute chemically activated cell
      IF (LOAD_BALANCE_ALGO == 1) THEN
         CALL DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES(NRECEIVE_CELLS)
      ELSE
         CALL DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES_ALGO2(NRECEIVE_CELLS)
      ENDIF
      ...
      ELSE
         CALL GATHER_CELLS_FROM_MPI_PROCESSES_ALGO2(DT)
      ENDIF
      IF(ALLOCATED(DZ_F0)) DEALLOCATE(DZ_F0)
      IF (STOP_STATUS/=NO_STOP) RETURN
   ENDIF !NCHEM_ACTIVE_CELLS_GLOBAL >0

ELSE DO_CHEM_LOAD_BALANCE_IF
   
   IF (NCHEM_ACTIVE_CELLS >0) THEN
      ! Serial chemistry:
      MESH_LOOP_SERIAL : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL POINT_TO_MESH(NM)
         !$OMP PARALLEL PRIVATE(JCC,I,J,K,CC,ZZ_GET, PRES, DZZ, DZ_F0, RRTMP0, MOLPCM3,STOP_STATUS, CHEM_SUBIT_TMP, &
         !$OMP REAC_SOURCE_TERM_TMP, Q_REAC_TMP)
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_GAS_CONSTANT,GET_MASS_FRACTION_ALL,GET_SPECIFIC_HEAT,GET_MOLECULAR_WEIGHT, &
                              GET_SENSIBLE_ENTHALPY_Z,IS_REALIZABLE
USE COMPLEX_GEOMETRY, ONLY : CC_CGSC, CC_GASPHASE
INTEGER :: I,J,K,N,CHEM_SUBIT_TMP, ICC, JCC, NCELL
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
...
SUBROUTINE CHECK_CHEMICALLY_ACTIVE_STATE (ZZ_GET, TMP, I, J , K, CHEM_ACTIVE)
...
IF (CC_IBM) THEN ! Check if cell is regular gas phase cell, if not return.
...
NCHEM_ACTIVE_CELLS = NCHEM_ACTIVE_CELLS_RANK_WISE(MY_RANK+1)
CELLS_PER_RANK = CEILING(REAL(NCHEM_ACTIVE_CELLS,EB)/REAL(N_MPI_PROCESSES,EB))
CELLS_ALLOC    = CELLS_PER_RANK * N_MPI_PROCESSES

ALLOCATE(NCELLS_TO_SEND_ARRAY(N_MPI_PROCESSES)); NCELLS_TO_SEND_ARRAY=0
...
IF (NCHEM_ACTIVE_CELLS > 0) THEN
   ...
   IF (MY_RANK == 0) THEN
      RANK_TO = 1
   ELSE
      RANK_TO = MOD(SUM(NCHEM_ACTIVE_CELLS_RANK_WISE(1:MY_RANK)), N_MPI_PROCESSES)+1
   ENDIF
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      CALL POINT_TO_MESH(NM)
      DO K=1,KBAR
   ...
   ALLOCATE(NEW_INDEX(NCHEM_ACTIVE_CELLS)); NEW_INDEX=-1
...
ALLOCATE(NCELLS_TO_RECV_ARRAY(1:N_MPI_PROCESSES)); NCELLS_TO_RECV_ARRAY=0
CALL MPI_ALLTOALL(NCELLS_TO_SEND_ARRAY,1,MPI_INTEGER,NCELLS_TO_RECV_ARRAY,1,MPI_INTEGER,MPI_COMM_WORLD,IERR)

 
! Now send the actual data:
ALLOCATE(SEND_DISPS(N_MPI_PROCESSES),REVC_DISPS(N_MPI_PROCESSES))
...
CALL MPI_ALLTOALLV(REALS_TO_SEND_ARRAY,NCELLS_TO_SEND_ARRAY*NVAR_TO_SEND,SEND_DISPS,MPI_DOUBLE_PRECISION,&
                   REALS_TO_RECV_ARRAY,NCELLS_TO_RECV_ARRAY*NVAR_TO_SEND,REVC_DISPS,MPI_DOUBLE_PRECISION,&
                   MPI_COMM_WORLD,IERR)
...
INTEGER, INTENT(OUT) :: NRECEIVE_CELLS
INTEGER :: I, J , K, NM, ICC, JCC, INDX, SEQ, RANK_TO,IERR, NP, STARTINDX
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES)
REAL(EB) :: PRES, TNOW, AVG
TYPE (CC_CUTCELL_TYPE), POINTER :: CC
TYPE (MPI_REQUEST), ALLOCATABLE, DIMENSION(:) :: REQ0
INTEGER :: N_REQ0, STARTLOC
...
SORTED_DIFF_FROM_AVG = 0
NCELLS_TO_SEND_ARRAY_ALGO2=0
NCELLS_TO_RECV_ARRAY_ALGO2=0
TOT_RANK_TO_COMM = 0

TAG = 111

! SORT NCHEM_ACTIVE_CELLS_RANK_WISE and get the INDEX
SORTED_RANK_INDX = [(I, I = 1, N_MPI_PROCESSES)]
CALL QUICKSORT(NCHEM_ACTIVE_CELLS_RANK_WISE, SORTED_RANK_INDX, 1, N_MPI_PROCESSES, 1)

! Fill the reverse map to point from MY_RANK to SORTED_RANK_INDX
AVG_PIVOT_INDX = -1
AVG = REAL(SUM(NCHEM_ACTIVE_CELLS_RANK_WISE), EB)/REAL(N_MPI_PROCESSES, EB)
REM = MOD(SUM(NCHEM_ACTIVE_CELLS_RANK_WISE), N_MPI_PROCESSES)
CELLS_PER_SORTED_RANK = FLOOR(AVG)
DO I=1,REM ! Distribute the remainder
   CELLS_PER_SORTED_RANK(I) = CELLS_PER_SORTED_RANK(I)+1
ENDDO

DO I = 1, N_MPI_PROCESSES
   SORTED_RANK_INDX_MAP(SORTED_RANK_INDX(I)) = I
   SORTED_DIFF_FROM_AVG(I) = NCHEM_ACTIVE_CELLS_RANK_WISE(SORTED_RANK_INDX(I)) - CELLS_PER_SORTED_RANK(I)
   IF (AVG_PIVOT_INDX == -1 .AND. &
      NCHEM_ACTIVE_CELLS_RANK_WISE(SORTED_RANK_INDX(I)) < AVG) THEN
      AVG_PIVOT_INDX = I
   ENDIF
END DO

!IF (MY_RANK == 0) THEN
!   WRITE(LU_ERR,*)"AVG_PIVOT_INDX=", AVG_PIVOT_INDX
!   DO I = 1, N_MPI_PROCESSES
!      WRITE(LU_ERR,*)"NCHEM_ACTIVE_CELLS, RANK, CELLS_PER_RANK, DIFF=",NCHEM_ACTIVE_CELLS_RANK_WISE(SORTED_RANK_INDX(I)), &
!         SORTED_RANK_INDX(I), CELLS_PER_SORTED_RANK(I), SORTED_DIFF_FROM_AVG(I)
!   ENDDO
!ENDIF

! These processes will only send as they have more cells than CELLS_PER_SORTED_RANK(I). Others will receive
IF (SORTED_RANK_INDX_MAP(MY_RANK+1) < AVG_PIVOT_INDX) THEN

   ! Find the processes where this process will send cells. Stored in NCELLS_TO_SEND_ARRAY
   SUM_PREV_SORTED_RANKS = 0
   DO I = 1, SORTED_RANK_INDX_MAP(MY_RANK+1)-1
      SUM_PREV_SORTED_RANKS = SUM_PREV_SORTED_RANKS + SORTED_DIFF_FROM_AVG(I)
   ENDDO
   PREVSUM = 0 ! Cells assigned from prev MPI processes
   CURSUM = 0 ! Cells assined to processes so far while iterating
   TOT_CELLS_TO_SEND = SORTED_DIFF_FROM_AVG(SORTED_RANK_INDX_MAP(MY_RANK+1))
   TOT_RANK_TO_COMM = 0
   DO I = N_MPI_PROCESSES, AVG_PIVOT_INDX, -1
      IF (CURSUM >= TOT_CELLS_TO_SEND) EXIT
      IF (SORTED_DIFF_FROM_AVG(I)==0) CYCLE
      PREVSUM = PREVSUM + SORTED_DIFF_FROM_AVG(I)
      IF (ABS(PREVSUM) >=  SUM_PREV_SORTED_RANKS) THEN
         STILL_TO_FILL = ABS(PREVSUM) - SUM_PREV_SORTED_RANKS - CURSUM
         IF (STILL_TO_FILL > 0) THEN
            PROC_CELLS_TO_SEND = MIN(STILL_TO_FILL, TOT_CELLS_TO_SEND-CURSUM)
            TOT_RANK_TO_COMM = TOT_RANK_TO_COMM+1
            NCELLS_TO_SEND_ARRAY_ALGO2(TOT_RANK_TO_COMM,1) = SORTED_RANK_INDX(I)-1
            NCELLS_TO_SEND_ARRAY_ALGO2(TOT_RANK_TO_COMM,2) = PROC_CELLS_TO_SEND
            CURSUM = CURSUM + PROC_CELLS_TO_SEND !Cells assigned for transfer so far
         ENDIF
      ENDIF
   ENDDO
   ! Assign self
   ...
   DO NP =1, TOT_RANK_TO_COMM
      IF (NP ==1) THEN
         NCELLS_TO_SEND_ARRAY_ALGO2(NP,3) = NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)
      ELSE
         NCELLS_TO_SEND_ARRAY_ALGO2(NP,3) = NCELLS_TO_SEND_ARRAY_ALGO2(NP-1,3) + NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)
      ENDIF
   ENDDO
   ...
   NRECEIVE_CELLS = NCELLS_TO_SEND_ARRAY_ALGO2(TOT_RANK_TO_COMM,2) !Only self for sending processes

   ! Now send
   ALLOCATE(REQ0(TOT_RANK_TO_COMM-1))
   N_REQ0 = 0
   DO NP =1, TOT_RANK_TO_COMM-1 ! The last one is self
      ...
      IF (NP ==1) THEN
         STARTLOC = 1
      ELSE
         STARTLOC = NCELLS_TO_SEND_ARRAY_ALGO2(NP-1,3)+1
      ENDIF
      CALL MPI_ISEND(REALS_TO_SEND_ARRAY(1,STARTLOC),NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)*NVAR_TO_SEND, &
               MPI_DOUBLE_PRECISION,NCELLS_TO_SEND_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO



ELSE IF(SORTED_RANK_INDX_MAP(MY_RANK+1) >= AVG_PIVOT_INDX .AND. &
        CELLS_PER_SORTED_RANK(SORTED_RANK_INDX_MAP(MY_RANK+1))>0) THEN ! Only recieve.

   ! Decide from which MPI processes the current processes will recieve cells.
   SUM_PREV_SORTED_RANKS = 0
   DO I = N_MPI_PROCESSES, SORTED_RANK_INDX_MAP(MY_RANK+1)+1, -1
      SUM_PREV_SORTED_RANKS = SUM_PREV_SORTED_RANKS + SORTED_DIFF_FROM_AVG(I)
   ENDDO
   PREVSUM = 0
   CURSUM = 0
   TOT_CELLS_TO_RECEIVE = ABS(SORTED_DIFF_FROM_AVG(SORTED_RANK_INDX_MAP(MY_RANK+1)))
   TOT_RANK_TO_COMM = 0
   IF(TOT_CELLS_TO_RECEIVE > 0) THEN
      DO I = 1, AVG_PIVOT_INDX-1
         IF (CURSUM >= TOT_CELLS_TO_RECEIVE) EXIT
         PREVSUM = PREVSUM + SORTED_DIFF_FROM_AVG(I)
         IF (PREVSUM > ABS(SUM_PREV_SORTED_RANKS)) THEN
            STILL_TO_RECEIVE = PREVSUM - ABS(SUM_PREV_SORTED_RANKS) - CURSUM
            IF (STILL_TO_RECEIVE > 0) THEN
               PROC_CELLS_TO_RECEIVE = MIN(STILL_TO_RECEIVE, TOT_CELLS_TO_RECEIVE-CURSUM)
               TOT_RANK_TO_COMM = TOT_RANK_TO_COMM+1
               NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,1) = SORTED_RANK_INDX(I)-1
               NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,2) = PROC_CELLS_TO_RECEIVE
               CURSUM = CURSUM + PROC_CELLS_TO_RECEIVE !Cells assigned to current processor so far
            ENDIF
         ENDIF
      ENDDO
   ENDIF
   ...
   ! Assign self cells
   TOT_RANK_TO_COMM = TOT_RANK_TO_COMM +1
   NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,1) = MY_RANK
   NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,2) = NCHEM_ACTIVE_CELLS_RANK_WISE(MY_RANK+1)

   DO NP =1, TOT_RANK_TO_COMM
      IF (NP ==1) THEN
         NCELLS_TO_RECV_ARRAY_ALGO2(NP,3) = NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)
      ELSE
         NCELLS_TO_RECV_ARRAY_ALGO2(NP,3) = NCELLS_TO_RECV_ARRAY_ALGO2(NP-1,3) + NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)
      ENDIF
   ENDDO

   ! Now recieve
   ALLOCATE(REALS_TO_RECV_ARRAY(NVAR_TO_SEND,NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,3))); REALS_TO_RECV_ARRAY=0._EB
   ALLOCATE(REQ0(TOT_RANK_TO_COMM-1))
   N_REQ0 = 0
   DO NP =1, TOT_RANK_TO_COMM-1
      N_REQ0 = N_REQ0 + 1;
      IF (NP ==1) THEN
         STARTLOC = 1
      ELSE
         STARTLOC = NCELLS_TO_RECV_ARRAY_ALGO2(NP-1,3)+1
      ENDIF
      CALL MPI_IRECV(REALS_TO_RECV_ARRAY(1,STARTLOC),NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)*NVAR_TO_SEND, &
               MPI_DOUBLE_PRECISION,NCELLS_TO_RECV_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO

   NRECEIVE_CELLS=NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,3) !Self + receiving cells

   ! Set the self cells as receiving cells.
   ALLOCATE(IS_HANDLED_BY_CURR_PROC(NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,2))); IS_HANDLED_BY_CURR_PROC = 0
   ALLOCATE(CHEM_SEQ_ARR(NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,2))); CHEM_SEQ_ARR = 0
   IF (TOT_RANK_TO_COMM ==1) THEN
      STARTINDX = 0
   ELSE
      STARTINDX = NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM-1,3)
   ENDIF
   SEQ = 0
   IF (NCHEM_ACTIVE_CELLS_RANK_WISE(MY_RANK+1) > 0) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL POINT_TO_MESH(NM)
         DO K=1,KBAR
            DO J=1,JBAR
...
IF (ALLOCATED(REALS_TO_SEND_ARRAY))  DEALLOCATE(REALS_TO_SEND_ARRAY)
IF(ALLOCATED(REQ0)) DEALLOCATE(REQ0)

T_CHEM_COMM = T_CHEM_COMM+CURRENT_TIME() - TNOW

END SUBROUTINE DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES_ALGO2
...
ENDDO
ALLOCATE(RESULTS_TO_RECV_ARRAY(NVAR_TO_RECEIVE,MAX(NCHEM_ACTIVE_CELLS,1))); RESULTS_TO_RECV_ARRAY=0._EB
CALL MPI_ALLTOALLV(RESULTS_TO_SEND_ARRAY(1,1),NCELLS_TO_RECV_ARRAY*NVAR_TO_RECEIVE,SEND_DISPS,MPI_DOUBLE_PRECISION,&
                   RESULTS_TO_RECV_ARRAY(1,1),NCELLS_TO_SEND_ARRAY*NVAR_TO_RECEIVE,REVC_DISPS,MPI_DOUBLE_PRECISION,&
                   MPI_COMM_WORLD,IERR)
DEALLOCATE(RESULTS_TO_SEND_ARRAY,NCELLS_TO_RECV_ARRAY,NCELLS_TO_SEND_ARRAY, SEND_DISPS,REVC_DISPS)
...
IF (SORTED_RANK_INDX_MAP(MY_RANK+1) < AVG_PIVOT_INDX) THEN ! Recieve the chemistry cells 
   ...
   DO NP =1, TOT_RANK_TO_COMM-1 ! For sender it includes self.
      ...
      ELSE
         STARTLOC = NCELLS_TO_SEND_ARRAY_ALGO2(NP-1,3)+1
      ENDIF
      CALL MPI_IRECV(RESULTS_TO_RECV_ARRAY(1,STARTLOC),NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)*NVAR_TO_RECEIVE, &
            MPI_DOUBLE_PRECISION,NCELLS_TO_SEND_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO


ELSE IF(SORTED_RANK_INDX_MAP(MY_RANK+1) >= AVG_PIVOT_INDX) THEN ! Send the chemistry cells
   ALLOCATE(REQ0(TOT_RANK_TO_COMM))
   N_REQ0 = 0
   DO NP =1, TOT_RANK_TO_COMM-1
      N_REQ0 = N_REQ0 + 1;
      ...
      ELSE
         STARTLOC = NCELLS_TO_RECV_ARRAY_ALGO2(NP-1,3)+1
      ENDIF
      CALL MPI_ISEND(RESULTS_TO_SEND_ARRAY(1,STARTLOC),NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)*NVAR_TO_RECEIVE, &
            MPI_DOUBLE_PRECISION,NCELLS_TO_RECV_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO
ENDIF
IF (N_REQ0>0) CALL MPI_WAITALL(N_REQ0,REQ0(1:N_REQ0),MPI_STATUSES_IGNORE,IERR)
...
IF (ALLOCATED(CHEM_SEQ_ARR)) DEALLOCATE(CHEM_SEQ_ARR)
IF (ALLOCATED(IS_HANDLED_BY_CURR_PROC)) DEALLOCATE(IS_HANDLED_BY_CURR_PROC)
IF (ALLOCATED(RESULTS_TO_RECV_ARRAY)) DEALLOCATE(RESULTS_TO_RECV_ARRAY)
IF (ALLOCATED(RESULTS_TO_SEND_ARRAY)) DEALLOCATE(RESULTS_TO_SEND_ARRAY)

END SUBROUTINE GATHER_CELLS_FROM_MPI_PROCESSES_ALGO2
...
REAL(EB) :: MW, RHO_IN, RHO_OUT, XXX, EQUIV, TNOW2
INTEGER :: NS
