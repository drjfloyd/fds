...
DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: ATOL
...
USE DVODECONS, ONLY: DVODE_MIN_ATOL
...
! In the two pairs of commented lines, top line uses the DVODE_JAC routine and bottom line uses a numerical Jacobian
         OPTIONS = SET_NORMAL_OPTS(DENSE_J=.TRUE.,ABSERR_VECTOR=ATOL,RELERR=RTOL,USER_SUPPLIED_JACOBIAN=.TRUE.)
!         OPTIONS = SET_NORMAL_OPTS(DENSE_J=.TRUE.,ABSERR_VECTOR=ATOL,RELERR=RTOL,USER_SUPPLIED_JACOBIAN=.FALSE.)
...
!          CALL DVODE_F90(DVODE_Y,N_TRACKED_SPECIES,ZZ_DVODE,T1,T2,ITASK,ISTATE,OPTIONS)
         ZZ_MIXED = REAL(ZZ_DVODE,EB)
      CASE (DASSL_SOLVER)
         Q_REAC_SUB = 0._EB
         DT_SUB = DT
         DO NS =1,N_TRACKED_SPECIES
            DASSL_A(NS) = MAX(DVODE_MIN_ATOL,RICHARDSON_ERROR_TOLERANCE*ZZ_MIXED(NS))
         ENDDO
         CALL DVODES_PRECALC(ZZ_MIXED,RHO_IN,TMP_IN)
