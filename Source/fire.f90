...
DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: ATOL,RTOL,LOWER_BOUND,UPPER_BOUND
REAL(EB), ALLOCATABLE, DIMENSION(:) :: RWORK
...
IF (.NOT. DVODE_INIT) THEN
   IF (COMBUSTION_ODE_SOLVER == DVODES_SOLVER) THEN
      ...
      LOWER_BOUND = DBLE(-ZZ_MIN_GLOBAL)
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (DVODES_SOLVER)
         ...
         DO NS =1,N_TRACKED_SPECIES
            RTOL(NS) = DBLE(SPECIES_MIXTURE(NS)%ODE_REL_ERROR)
            ATOL(NS) = DBLE(MAX(ODE_MIN_ATOL,MIN(1._EB,ZZ_MIXED(NS)*10._EB)*SPECIES_MIXTURE(NS)%ODE_REL_ERROR*1.E-6))
         ENDDO
         OPTIONS = SET_OPTS(DENSE_J=.TRUE.,ABSERR_VECTOR=ATOL,RELERR_VECTOR=RTOL,USER_SUPPLIED_JACOBIAN=.TRUE.,MXSTEP=50000,&
                            CONSTRAINED=BOUNDED_COMPONENTS,CLOWER=LOWER_BOUND,CUPPER=UPPER_BOUND)
         CALL DVODES_PRECALC(ZZ_MIXED,RHO_IN,TMP_IN)
         T1 = 0.D0
         T2 = DBLE(DT)
         ISTATE=1
         !IF (T1 > TWO_EPSILON_EB) ISTATE = 2
...
!   ! Check for species with concentration exponents
   DO NS=1,RN%N_SPEC
      IF(ZZ_TMP(YP2ZZ(RN%N_S_INDEX(NS))) < SPECIES_MIXTURE(YP2ZZ(RN%N_S_INDEX(NS)))%ODE_REL_ERROR*ZZ_MIN_GLOBAL) CYCLE REACTION_LOOP
   ENDDO
   ! dZ/dt, FDS Tech Guide, Eq. (5.38)
   DZ_F = DZ_F0(I)
   DO NS=1,RN%N_SPEC
