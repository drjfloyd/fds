...
DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: ATOL,LOWER_BOUND,UPPER_BOUND
DOUBLE PRECISION :: RTOL
REAL(EB), ALLOCATABLE, DIMENSION(:) :: DASSL_A,DASSL_R,RWORK
LOGICAL :: WO=.FALSE.,DVODE_INIT=.FALSE.
INTEGER :: INFO2(15)=0,IPAR(1),LIW,LRW
...
! Set DVODES options
IF (.NOT. DVODE_INIT) THEN
   IF (COMBUSTION_ODE_SOLVER == DVODES_SOLVER) THEN
      DVODE_INIT = .TRUE.
      ALLOCATE(ATOL(N_TRACKED_SPECIES))
      ALLOCATE(BOUNDED_COMPONENTS(N_TRACKED_SPECIES))
      DO ICC=1,N_TRACKED_SPECIES
         BOUNDED_COMPONENTS(ICC) = ICC
      ENDDO
      ALLOCATE(LOWER_BOUND(N_TRACKED_SPECIES))
      LOWER_BOUND = DBLE(-TWO_EPSILON_EB)
      ALLOCATE(UPPER_BOUND(N_TRACKED_SPECIES))
      UPPER_BOUND = 1000000.D0
   ELSEIF (COMBUSTION_ODE_SOLVER == DASSL_SOLVER) THEN
      DVODE_INIT = .TRUE.
      ALLOCATE(DASSL_A(N_TRACKED_SPECIES))
      ALLOCATE(DASSL_R(N_TRACKED_SPECIES))
      DASSL_R = RICHARDSON_ERROR_TOLERANCE
      ...
      ALLOCATE(IWORK(LIW))
      IWORK = 0
      LRW = 40+9*N_TRACKED_SPECIES+N_TRACKED_SPECIES**2
      ALLOCATE(RWORK(LRW))
      RWORK = 0._EB
   ENDIF
ENDIF
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZETA_1,ZETA_2,ZETA_4,D_F,TMP_IN,C_U,DT_SUB_OLD,TNOW2,ERR_EST(N_TRACKED_SPECIES),ERR_TOL(N_TRACKED_SPECIES)
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (RK2_RICHARDSON) ! Finite-rate (or mixed finite-rate/fast) chemistry
         ...
         DO NS = 1,N_TRACKED_SPECIES
            ERR_TOL(NS) = MAX(0.1_EB*ZZ_MIN_GLOBAL,RICHARDSON_ERROR_TOLERANCE*ZZ_MIXED(NS),ODE_MIN_ATOL)
         ENDDO
         RICH_EX_LOOP: DO RICH_ITER = 1,RICH_ITER_MAX
            ...
            IF (N_FIXED_CHEMISTRY_SUBSTEPS<0) THEN
               ...
               DT_SUB_NEW = MIN(MAX(DT_SUB*MINVAL(ERR_TOL/(ERR_EST+TINY_EB))**(0.25_EB),DT_SUB_MIN),DT-DT_ITER) ! (E.9)
               IF (ALL(ERR_EST<=ERR_TOL) .OR. ABS(DT_SUB_OLD/DT_SUB_NEW-1._EB) <= 1.E-2_EB) EXIT RICH_EX_LOOP
            ENDIF
         ENDDO RICH_EX_LOOP
      ...
      CASE (DVODES_SOLVER)
         ...
         DO NS =1,N_TRACKED_SPECIES
            ATOL(NS) = MAX(0.1D0*DBLE(ZZ_MIN_GLOBAL),ODE_MIN_ATOL,DBLE(RICHARDSON_ERROR_TOLERANCE*ZZ_MIXED(NS)))
         ENDDO
         OPTIONS = SET_OPTS(DENSE_J=.TRUE.,ABSERR_VECTOR=ATOL,RELERR=RTOL,USER_SUPPLIED_JACOBIAN=.TRUE.,MXSTEP=50000,&
      ...
      CASE (DASSL_SOLVER)
         ...
         DT_SUB = DT
         DO NS =1,N_TRACKED_SPECIES
            DASSL_A(NS) = MAX(0.1_EB*ZZ_MIN_GLOBAL,ODE_MIN_ATOL,RICHARDSON_ERROR_TOLERANCE*ZZ_MIXED(NS))
         ENDDO
         CALL DVODES_PRECALC(ZZ_MIXED,RHO_IN,TMP_IN)
         CALL DASSL_YP(ZZ_MIXED,A1)
         T1 = T-DT
         T2 = T
         CALL DDASSL(DASSL_RESID,N_TRACKED_SPECIES,T1,ZZ_MIXED,A1,T2,INFO2,DASSL_R,DASSL_A,ISTATE,RWORK,LRW,IWORK,LIW,RPAR,&
                     IPAR,DASSL_JAC)
         IF (INFO2(1) < 0) THEN
            WRITE(*,*) 'DASSL:',INFO2(1)
            STOP
...
REAL(EB) :: DZ_F,ZZ_TMP(1:N_TRACKED_SPECIES),X_Y(1:N_SPECIES),X_Y_SUM,T1
INTEGER :: I,NS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
REAL(EB) :: DZ_F,DZZ_TMP,ZZ_TMP(N_TRACKED_SPECIES),X_Y(N_SPECIES),X_Y_SUM,T1
...
REAL(EB) :: DZ_F,X_Y(1:N_SPECIES),X_Y_SUM
INTEGER :: I,NS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
REAL(EB) :: DZ_F,DZZ_TMP,X_Y(N_SPECIES),X_Y_SUM,T1,R1,CJ1,ZZ1
INTEGER :: I,NS, NS2,I1
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
