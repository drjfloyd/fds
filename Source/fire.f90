...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (DVODES_SOLVER)
         ...
         ENDDO
         OPTIONS = SET_OPTS(DENSE_J=.TRUE.,ABSERR_VECTOR=ATOL,RELERR_VECTOR=RTOL,USER_SUPPLIED_JACOBIAN=.TRUE.,MXSTEP=100000)!,&
...
!         T1 = 0.D0
!         T2 = DBLE(DT)
!         CALL SDIRK4(N_TRACKED_SPECIES,DVODE_Y,T1,ZZ_DVODE,T2,0.D0, &
!                  RTOL,ATOL,1, &
!                  DVODE_JAC ,1,N_TRACKED_SPECIES,N_TRACKED_SPECIES, &
!                  MAS ,0,N_TRACKED_SPECIES,N_TRACKED_SPECIES, &
!                  SOLOUT,0, &
!                  WORK,2*N_TRACKED_SPECIES*2+12*N_TRACKED_SPECIES+7,IWORK,2*N_TRACKED_SPECIES+4,5*N_TRACKED_SPECIES+2,IDID)
!         ZZ_MIXED = REAL(ZZ_DVODE,EB)
      CASE (DASSL_SOLVER)
         Q_REAC_SUB = 0._EB
         DT_SUB = DT
