...
REAL(EB), ALLOCATABLE, DIMENSION(:) :: DZ_F0,K_INF0
REAL(EB) :: RRTMP0,RPAR(1),RHO_DV,TMP_DV,MOLPVR,MOLPVTB
...
IF (ALLOCATED(K_INF0)) DEALLOCATE(K_INF0)
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZZ_TEMP(1:N_TRACKED_SPECIES),REL_ERR(N_TRACKED_SPECIES),ABS_ERR(N_TRACKED_SPECIES),XX_MIXED(N_TRACKED_SPECIES)
INTEGER :: NR,NS,ITER,TVI,RICH_ITER,TIME_ITER,RICH_ITER_MAX,ITASK=1,ISTATE=1,COUNTER,COUNTER2
DOUBLE PRECISION :: XX_DVODE(N_TRACKED_SPECIES)
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (DVODE_SOLVER)
         Q_REAC_SUB = 0._EB
         DT_SUB = DT
         XX_DVODE = DBLE(RHO_IN*ZZ_MIXED/SPECIES_MIXTURE%MW)
         RHO_DV = RHO_IN
         TMP_DV = TMP_IN
         MOLPVR = P_STP/(R0*TMP_IN)
         MOLPVTB = PRES_IN/(R0*TMP_IN)
         DO NS =1,N_TRACKED_SPECIES
...
!                            CONSTRAINED=BOUNDED_COMPONENTS,CLOWER=LOWER_BOUND,CUPPER=UPPER_BOUND)
         ...
         COUNTER = 1
         DO
            CALL DVODE_F90(DVODE_Y,N_TRACKED_SPECIES,XX_DVODE,T1,T2,ITASK,ISTATE,OPTIONS,J_FCN=DVODE_JAC)
            IF (ISTATE == 2) EXIT
            WRITE(LU_ERR,*) 'ISTATE:',ISTATE,NM0,I0,J0,K0,COUNTER,RHO_IN,TMP_IN
            WRITE(LU_ERR,*) 'ZZ_IN:',ZZ_MIXED
            WRITE(LU_ERR,*) 'XX_NOW:',XX_DVODE
            IF (ISTATE == -1) THEN
               ISTATE = 3
               COUNTER = COUNTER + 1
               IF (COUNTER == 20) THEN
            ...
            ENDIF
         ENDDO
         ZZ_MIXED = REAL(XX_DVODE*SPECIES_MIXTURE%MW,EB)
         ZZ_MIXED = ZZ_MIXED / SUM(ZZ_MIXED)
      CASE (CVODE_SOLVER)
         T1 = T - DT
         T2 = T
         DO NS =1,N_TRACKED_SPECIES
      ...
      CASE (DASSL_SOLVER)
         ...
         ENDDO
         XX_MIXED = RHO_IN*ZZ_MIXED/SPECIES_MIXTURE%MW
         CALL DVODES_PRECALC
         CALL DASSL_YP(XX_MIXED,A1)
         T1 = 0._EB!T-DT
         T2 = DT!T
         COUNTER = 0
         COUNTER2 = 0
         INFO2(1) = 0
         DASSL_LOOP: DO
            CALL DDASSL(DASSL_RESID,N_TRACKED_SPECIES,T1,XX_MIXED,A1,T2,INFO2,REL_ERR,ABS_ERR,ISTATE,RWORK,LRW,IWORK,LIW,RPAR,&
                        IPAR,DASSL_JAC)
            SELECT CASE(ISTATE)
               CASE(1:)
                  EXIT DASSL_LOOP
               CASE(-1)
                  ...
                  IF (COUNTER == 10) THEN
                     ...
                     ELSE
                        ...
                        INFO2(1) = 0
                        XX_MIXED = RHO_IN*ZZ_GET/SPECIES_MIXTURE%MW
                        T1 = 0._EB!T-DT
                        T2 = DT!T
                        COUNTER = 0
                        CALL DASSL_YP(XX_MIXED,A1)
            ...
            END SELECT
         ENDDO DASSL_LOOP
         ZZ_MIXED = XX_MIXED*SPECIES_MIXTURE%MW
         ZZ_MIXED = ZZ_MIXED / SUM(ZZ_MIXED)              
...
REAL(EB) :: DZ_F,YY_PRIMITIVE(1:N_SPECIES),MW,DT_TMP(1:N_TRACKED_SPECIES),DT_MIN,DT_LOC,&
            ...
            K_INF,K_0,P_RI,FCENT,C_I,THIRD_BODY_ENHANCEMENT
      ...
      SLOW_REAC_LOOP: DO OUTER_IT=1,N_REACTIONS
         ...
         REACTION_LOOP_2: DO I=1,N_REACTIONS
            ...
            IF (DZ_F0(I) < 0._EB) THEN
               ...
               DZ_F0(I) = K_INF
               IF (RN%REVERSE) & ! compute equilibrium constant
                  DZ_F0(I) = DZ_F0(I)*EXP(RN%DELTA_G(MIN(I_MAX_TEMP,NINT(TMP_0)))/TMP_0)*MOLPVR**RN%C0_EXP
            ENDIF
            DZ_F = DZ_F0(I)
            IF (RN%THIRD_BODY) THEN
               IF (RN%N_THIRD > 0) THEN
                  ...
                  ENDDO
                  THIRD_BODY_ENHANCEMENT = MOLPVTB * SUM(X_Y)/X_Y_SUM
               ELSE
                  THIRD_BODY_ENHANCEMENT = MOLPVTB
               ENDIF                                   
               IF (RN%REACTYPE==FALLOFF_LINDEMANN_TYPE .OR. RN%REACTYPE==FALLOFF_TROE_TYPE) THEN
                  ...
                  DZ_F = DZ_F * C_I   
               ELSEIF (RN%REACTYPE==THREE_BODY_ARRHENIUS_TYPE) THEN
                  DZ_F = DZ_F * THIRD_BODY_ENHANCEMENT
               ENDIF
            ENDIF
            DO NS=1,RN%N_SPEC
...
SUBROUTINE DVODES_PRECALC

INTEGER :: I
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()

RRTMP0 = 1._EB/(R0*TMP_DV)

DO I=1,N_REACTIONS
   RN=>REACTION(I)
   K_INF0(I) = RN%A_SI*TMP_DV**RN%N_T*EXP(-RN%E*RRTMP0)
   DZ_F0(I) = K_INF0(I)
   IF (RN%REVERSE) & ! compute equilibrium constant
      DZ_F0(I) = DZ_F0(I)*EXP(RN%DELTA_G(MIN(I_MAX_TEMP,NINT(TMP_DV)))/TMP_DV)*MOLPVR**RN%C0_EXP
ENDDO
...
REAL(EB) :: DZ_F,ZZ_TMP(1:N_TRACKED_SPECIES),T1,MIN_SPEC(N_TRACKED_SPECIES),&
            MW,FCENT,C_I,THIRD_BODY_ENHANCEMENT,K_0,P_RI,KG
INTEGER :: I,NS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
REACTION_LOOP: DO I=1,N_REACTIONS
   ...
   IF (RN%THIRD_BODY) THEN
      IF (RN%N_THIRD > 0) THEN
         THIRD_BODY_ENHANCEMENT = DOT_PRODUCT(ZZ_OLD(1:N_SPECIES),RN%THIRD_EFF(1:N_SPECIES))
      ELSE
         THIRD_BODY_ENHANCEMENT = MOLPVTB
      ENDIF                                   
      IF (RN%REACTYPE==FALLOFF_LINDEMANN_TYPE .OR. RN%REACTYPE==FALLOFF_TROE_TYPE) THEN
         K_0 = RN%A_LOW_PR*TMP_DV**(RN%N_T_LOW_PR)*EXP(-RN%E_LOW_PR*RRTMP0)
         P_RI = K_0/K_INF0(I)*THIRD_BODY_ENHANCEMENT
         FCENT = CALC_FCENT(TMP_DV,P_RI,I)
         C_I = P_RI/(1._EB+P_RI)*FCENT
         DZ_F = DZ_F * C_I   
      ELSEIF (RN%REACTYPE==THREE_BODY_ARRHENIUS_TYPE) THEN
         DZ_F = DZ_F * THIRD_BODY_ENHANCEMENT
   ...
   DO NS=1,RN%N_SMIX_FR
      DZZ(RN%NU_INDEX(NS)) = DZZ(RN%NU_INDEX(NS)) + DBLE(RN%NU_NN(RN%NU_INDEX(NS))*DZ_F)
   ENDDO
ENDDO REACTION_LOOP
...
REAL(EB) :: DZ_F,DZZ_TMP,ZZ_TMP(N_TRACKED_SPECIES),T1,MIN_SPEC(N_TRACKED_SPECIES),&
            MW,FCENT,C_I,THIRD_BODY_ENHANCEMENT,K_0,P_RI,KG
...
!   ! Check for consumed species
   ...
   DZ_F = DZ_F0(I)
   DO NS=1,RN%N_SPEC
      IF (ZZ_TMP(YP2ZZ(RN%N_S_INDEX(NS))) < MIN_SPEC(YP2ZZ(RN%N_S_INDEX(NS)))) CYCLE REACTION_LOOP
      DZ_F = DZ_F*ZZ_TMP(YP2ZZ(RN%N_S_INDEX(NS)))**RN%N_S(NS)
   ENDDO
   ...
   DZZ_TMP = 0._EB
   DO NS=1,RN%N_SMIX_FR
      DZZ_TMP = RN%NU_NN(RN%NU_INDEX(NS))*DZ_F
      DO NS2 = 1, RN%N_SPEC
...
REAL(EB) :: DZ_F,MIN_SPEC(N_TRACKED_SPECIES),&
            MW,FCENT,C_I,THIRD_BODY_ENHANCEMENT,K_0,P_RI,KG
INTEGER :: I,NS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
REACTION_LOOP: DO I=1,N_REACTIONS
   ...
   DO NS=1,RN%N_SMIX_FR
      DZZ(RN%NU_INDEX(NS)) = DZZ(RN%NU_INDEX(NS)) + RN%NU_NN(RN%NU_INDEX(NS))*DZ_F
   ENDDO
ENDDO REACTION_LOOP
...
REAL(EB) :: DZ_F,DZZ_TMP,T1,R1,CJ1,ZZ1,MIN_SPEC(N_TRACKED_SPECIES),&
            MW,FCENT,C_I,THIRD_BODY_ENHANCEMENT,K_0,P_RI,KG
INTEGER :: I,NS, NS2,I1
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
