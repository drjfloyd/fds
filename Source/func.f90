...
!> \brief Functions needed by DASSL solver

MODULE DASSL_FUNCTIONS
USE PRECISION_PARAMETERS
USE DASSL_CONSTANTS
USE TYPES, ONLY: REACTION_TYPE, REACTION, SPECIES_MIXTURE
USE  GLOBAL_CONSTANTS, ONLY: N_REACTIONS, N_TRACKED_SPECIES, N_SPECIES, ZZ_MIN_GLOBAL, R0, I_MAX_TEMP
IMPLICIT NONE

CONTAINS

!> \brief Generate Yprime vector for use in DASSL

SUBROUTINE YPRIME_CALC(YP)
USE PHYSICAL_FUNCTIONS, ONLY: GET_MOLECULAR_WEIGHT
REAL(EB), INTENT(OUT) :: YP(N_TRACKED_SPECIES)
REAL(EB) :: ZZ_GET(N_TRACKED_SPECIES),MW,MOLPCM3,DZZ(N_TRACKED_SPECIES),DZ_F,X_Y_SUM,&
            X_Y(N_SPECIES),X_Y_1(N_SPECIES),RRTMP0
INTEGER :: I, NS
...
IF (CALC_AE_FLAG) THEN
   RRTMP0 = 1._EB/(R0*RN_TMP)
   DO I=1,N_REACTIONS
      RN=>REACTION(I)
      RN_AE(I) = RN%A_PRIME*RN_RHO**RN%RHO_EXPONENT*RN_TMP**RN%N_T*EXP(-RN%E*RRTMP0)
      IF (RN%THIRD_BODY .AND. RN%N_THIRD <=0) THEN
         IF (MW < 0._EB) THEN
            CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
            MOLPCM3 = RN_RHO/MW*0.001_EB ! mol/cm^3
            RN_AE(I) = RN_AE(I) * MOLPCM3
         ENDIF
      ENDIF
      IF(RN%REVERSE) THEN ! compute equilibrium constant
         IF (MW < 0._EB) THEN
            ...
            MOLPCM3 = RN_RHO/MW*0.001_EB ! mol/cm^3
         ENDIF
         RN_AE(I) = RN_AE(I) * EXP(RN%DELTA_G(MIN(I_MAX_TEMP,NINT(RN_TMP)))/RN_TMP)*MOLPCM3**RN%C0_EXP
      ENDIF
   ENDDO
ENDIF

IF (ANY(REACTION%THIRD_BODY) .AND. MW < 0._EB) THEN
   CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
   MOLPCM3 = RN_RHO/MW*0.001_EB ! mol/cm^3
   IF (ANY(REACTION%N_THIRD>0)) THEN
      X_Y = RN_ZZ/SPECIES_MIXTURE%MW
      X_Y_SUM = SUM(X_Y)
   ENDIF
ENDIF

REAC_LOOP: DO I=1,N_REACTIONS
   DZ_F = RN_AE(I)
      DO NS=1,RN%N_SPEC
         IF (RN_ZZ(RN%N_S_INDEX(NS))<ZZ_MIN_GLOBAL) CYCLE REAC_LOOP
         DZ_F = RN_ZZ(RN%N_S_INDEX(NS))**RN%N_S(NS)*DZ_F
      ENDDO
      IF (RN%N_THIRD > 0) THEN
         X_Y_1 = X_Y * RN%THIRD_EFF
         DZ_F = DZ_F * MOLPCM3 * SUM(X_Y_1)/X_Y_SUM
      ENDIF
      IF (DZ_F > TWO_EPSILON_EB) THEN
         DO NS=1,RN%N_SMIX_FR
            YP(RN%NU_INDEX(NS)) = YP(RN%NU_INDEX(NS)) + RN%NU_MW_O_MW_F_FR(NS)*DZ_F
         ENDDO
      ENDIF
ENDDO REAC_LOOP

END SUBROUTINE YPRIME_CALC

!> \brief Generate Yprime vector for use in DASSL

SUBROUTINE DASSL_RESID(T0,Y,Y_PRIME,DR,IRES,RPAR,IPAR)
REAL(EB), INTENT(OUT):: DR(N_TRACKED_SPECIES)
REAL(EB), INTENT(IN):: Y(N_TRACKED_SPECIES),Y_PRIME(N_TRACKED_SPECIES),T0,RPAR(*)
INTEGER, INTENT(IN):: IPAR(*)
INTEGER, INTENT(OUT):: IRES
INTEGER:: I1
REAL(EB) :: YP(N_TRACKED_SPECIES),T1,R1
