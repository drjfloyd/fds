...
CONTAINS
  ...
  SUBROUTINE DERIVATIVE(CVEC,FVEC)
    ...
    REAL(EB) :: R_F,X_Y_SUM,MIN_SPEC(N_TRACKED_SPECIES), KG,  TMP, RHO, &
    ...
    DO I =1, N_TRACKED_SPECIES
       RHO = RHO + SPECIES_MIXTURE(I)%MW * CVEC(I)
    ENDDO
    ...
    REACTION_LOOP: DO I=1,N_REACTIONS
       ...
       R_F = RN%A_SI*TMP**RN%N_T*EXP(-RN%E*RRTMP) !A_SI IS IN [KMOL/M3]^() UNIT.
       ...
       DO NS=1,RN%N_SMIX_FR
          STOICH_COEFF = RN%NU_MW_O_MW_F_FR(NS)* ABS(RN%NU_FUEL_0)  * &
                       SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW/SPECIES_MIXTURE(RN%NU_INDEX(NS))%MW
          FVEC(RN%NU_INDEX(NS)) = FVEC(RN%NU_INDEX(NS)) + STOICH_COEFF* R_F
       ENDDO
    ENDDO REACTION_LOOP
  ...
  REAL(EB) FUNCTION CALCFCENT(TMP, P_RI, RN)
     ...
     IF(RN%REACTYPE=='FALLOFF-TROE') THEN
        LOGFCENT = LOG10(MAX((1 - RN%A_TROE)*EXP(-TMP/RN%T3_TROE) &
                   ...
                   + EXP(-RN%T2_TROE/TMP),1.E-20_EB))
        C = -0.4_EB - 0.67_EB*LOGFCENT
        D = 0.14_EB
        N = 0.75_EB - 1.27_EB*LOGFCENT
        LOGPR = LOG10(MAX(P_RI, 1.E-20_EB))
        CALCFCENT = 10._EB**(LOGFCENT/(1 + ((LOGPR + C)/(N - D*(LOGPR + C)))**2))
     ENDIF
     RETURN
  ...
  SUBROUTINE JACOBIAN(CVEC,FVEC, JMAT)
    ...
    REAL(EB) :: R_F,DCVEC,X_Y_SUM,MIN_SPEC(N_TRACKED_SPECIES), KG,  TMP, RHO, &
                K_0, K_INF, P_RI, FCENT, C_I, RRTMP, THIRD_BODY_ENHANCEMENT,STOICH_COEFF
    INTEGER :: I,NS, NS2
    ...
    ! TO PREVENT UNUSED VARIABLE WARNING
    IF(DEBUG) THEN
       WRITE(LU_ERR,*)" SIZE OF FVEC ARRAY=", SIZE(FVEC)
    ENDIF
